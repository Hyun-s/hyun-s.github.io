...
class LLMProcessor:
    ...
    def classify_job(self, job_data: dict, categories: list) -> Optional[dict]:
        """
        Use LLM to classify job posting into categories and extract structured data
        """
        description = job_data.get('description', '')
        if not description or len(description) < 50:
            logger.warning(f"Description too short for classification: {job_data.get('title')}")
            return {
                "primary_category": "others",
                "subcategories": [],
                "skills": [],
                "confidence": 1.0,
                "reasoning": "공고 본문이 없거나 너무 짧아 분류가 불가능합니다."
            }
        ...
        prompt = f"""
        Analyze this job posting and classify it into the most appropriate category.
        ...
        Description: {description[:2000]} # Limit description for context
        ...
        - subcategories: array of relevant subcategory slugs (from keywords or skills)
        ...
        Example output:
        {{
            "primary_category": "vision",
            "subcategories": ["computer-vision"],
            "skills": ["Python", "PyTorch", "OpenCV"],
            "confidence": 0.89,
            "reasoning": "비전 관련 업무가 주를 이룹니다..."
        }}
        """
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "You are a job classification assistant."},
        ...
        try:
            logger.info(f"Attempting classification with local LLM ({self.model_name})")
            ...
            logger.info(f"Successfully classified using local LLM")
            return data
        except Exception as e:
            logger.error(f"Failed classification with local LLM: {e}")
            return None
    ...
    def summarize_job(self, job_title: str, company: str, description: str) -> Optional[Dict]:
        ...
        5. role_summary: 해당 채용의 주요 업무를 기술하여야함. (구체적인 업무의 내용이 들어가야함)
        6. key_requirements: 필수 자격 요건 (리스트)
        7. preferences: 주요 우대사항 (리스트)
